...
class Material {
    ...
    set shader(shader) {
        if (!shader.isBuilt) console.warn("Shader ", shader, " not built");
        this._shader = shader;
    }
    apply() {
        this._shader.bind();
        ...
        for (const [key, value] of Object.entries(this._data.float2))this._shader.setFloat2(key, value[0], value[1]);
        for (const [key, value] of Object.entries(this._data.float3))this._shader.setFloat3(key, value[0], value[1], value[2]);
        for (const [key, value] of Object.entries(this._data.mat4))this._shader.setMat4(key, value);
...
//# sourceMappingURL=index.2ce2039a.js.map
