class Shader {
    ...
    build() {
        ...
        if (!compiledVertexShader || !compiledFragmentShader) return false;
        this._shaderProgram = this._linkShaders(compiledVertexShader, compiledFragmentShader);
        if (this._shaderProgram) return true;
        return false;
    }
    ...
    createVertexArray(vertexBuffers) {
        ...
        vertexBuffers.forEach((vertexBuffer)=>{
            ...
            for(let i = 0; i < locations.length; i++)locatoins[i] = gl.getAttribLocation(veretxBuffer.layout[i].name);
    ...
    getUniformLocation(name) {
        const location = gl.getUniformLocation(this._shaderProgram, name);
        if (location == -1) console.log(name, " uniform not found");
        return location;
    }
    bind() {
...
//# sourceMappingURL=index.944cb514.js.map
