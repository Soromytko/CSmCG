class GameObject {
    ...
    set globalPosition(value) {
        ...
        if (this._parent) // DOTO
        glMatrix.vec3.sub(this._localPosition, value, this._parent._globalPosition);
        else glMatrix.vec3.copy(this._localPosition, this._globalPosition);
        this._updateMatrix();
    ...
    set localPosition(value) {
        ...
        } else glMatrix.vec3.copy(this._globalPosition, this._localPosition);
        this._updateChildrenRecursive();
    }
    get rotation() {
    ...
    getRelativeDirection(x, y, z) {
        ...
        glMatrix.mat4.translate(destMat, this._matrix, [
            x,
            y,
            z
        ]);
    ...
    _addChild(newChild) {
        ...
        if (index < 0) this._children.push(newChild);
    }
    ...
    _isChildRecursive(maybeChild) {
        if (this._children.indexOf(maybeChild) >= 0) return true;
        for(let i = 0; i < this._children.length; i++)return this._children[i]._isChildRecursive(maybeChild);
    }
    _updateMatrix() {
        ...
        glMatrix.mat4.rotate(this._matrix, this._matrix, rot[1], [
            0,
            1,
            0
        ]);
        glMatrix.mat4.rotate(this._matrix, this._matrix, rot[0], [
            1,
            0,
            0
        ]);
...
//# sourceMappingURL=index.5d8067f7.js.map
